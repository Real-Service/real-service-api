...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Log auth state changes (cookie detection removed for production)
  useEffect(() => {
    ...
    // Cookie detection has been removed for production
  }, [user, isLoading]);
  ...
  const login = async (data: LoginData) => {
    ...
    try {
      // Always use cookies for login to ensure compatibility with production
      const res = await fetch('/api/direct-login', {
        ...
        credentials: 'include'  // Always include cookies for production compatibility
      });
      ...
      // Parse response from cookie-based login
  ...
  const register = async (data: RegisterData) => {
    console.log('Attempting registration with cookies...');
    try {
      // First register the user - always use cookies
  ...
  const logout = async () => {
    ...
    try {
      // First call the mutation to notify the server
      await logoutMutation.mutateAsync();
      
      // Import and use the comprehensive logout utility
      const { performFullLogout, closeAllWebSockets } = await import('@/lib/logout-utils');
      
      // Close any active WebSockets first
      closeAllWebSockets();
      ...
      await performFullLogout({
        ...
        forceReload: true,
        onLogoutComplete: () => {
          // This will run just before redirect
          toast({
            title: 'Logged Out',
            description: 'You have been successfully logged out'
          });
          
          // Extra safety measures - also update in-memory state
          queryClient.setQueryData(['/api/user'], null);
      ...
      // Fallback manual logout if the utility fails
      try {
        ...
        // 2. Clear any items with auth-related names in both storages
        ...
        console.log('Fallback logout completed, redirecting to auth page');
      } catch (fallbackErr) {
        console.error('Even fallback logout failed:', fallbackErr);
      }
      
      // Redirect to auth page as a last resort
