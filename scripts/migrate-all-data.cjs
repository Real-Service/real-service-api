...
async function migrateContractorProfiles() {
  try {
    ...
    // First, check the column name for the contractor profiles table in target DB
    // Default user column name (we'll check if it exists)
    ...
    if (tableCheck.rows[0].exists) {
      // Check which column name exists in target
      const userIdColumnCheck = await targetPool.query(`
        SELECT column_name
      ...
      if (userIdColumnCheck.rows.length > 0) {
        userIdColumn = userIdColumnCheck.rows[0].column_name;
        log(`Found user ID column: "${userIdColumn}" in target contractor_profiles table`);
      }
    } else {
      log('contractor_profiles table does not exist in target DB. Creating it first...');
      // Create the table with proper snake_case column names to match the schema
      ...
      userIdColumn = 'user_id'; // We created it with snake_case
    }
    ...
    for (const profile of sourceProfiles) {
      ...
      const existingCheck = await targetPool.query(`SELECT id FROM contractor_profiles WHERE ${userIdColumn} = $1`, [profile.userId]);
      ...
      if (existingCheck.rows.length > 0) {
        ...
        for (const [key, value] of Object.entries(profile)) {
          if (key !== 'id' && key !== 'userId' && value !== null) {
            ...
            const targetColumnName = key.replace(/([A-Z])/g, "_$1").toLowerCase();
            updateFields.push(`"${targetColumnName}" = $${paramIndex}`);
            values.push(value);
            paramIndex++;
          }
        }
        ...
        if (updateFields.length > 0) {
          const updateQuery = `
            ...
            WHERE ${userIdColumn} = $${paramIndex}
          `;
          await targetPool.query(updateQuery, values);
          log(`Updated contractor profile for user ${profile.userId}`);
        }
      } else {
        // Insert new profile with column name mapping
        const sourceKeys = Object.keys(profile);
        const targetKeys = sourceKeys.map(key => {
          // Convert camelCase to snake_case for column names
          return key.replace(/([A-Z])/g, "_$1").toLowerCase();
        });
        
        const formattedKeys = targetKeys.map(k => `"${k}"`).join(', ');
        const placeholders = sourceKeys.map((_, i) => `$${i + 1}`).join(', ');
        const values = Object.values(profile);
        
        await targetPool.query(`
          INSERT INTO contractor_profiles (${formattedKeys})
          VALUES (${placeholders})
        `, values);
  ...
  try {
    ...
    for (const job of sourceJobs) {
      ...
      if (existingCheck.rows.length > 0) {
        ...
        // Insert new job with column name mapping
        const sourceKeys = Object.keys(job);
        const targetKeys = sourceKeys.map(key => {
          // Convert camelCase to snake_case for column names
          return key.replace(/([A-Z])/g, "_$1").toLowerCase();
        });
        ...
        await targetPool.query(`
          INSERT INTO jobs (${formattedKeys})
          VALUES (${placeholders})
        `, values);
  ...
  try {
    ...
    for (const bid of sourceBids) {
      ...
      if (existingCheck.rows.length > 0) {
        ...
        // Insert new bid with column name mapping
        const sourceKeys = Object.keys(bid);
        const targetKeys = sourceKeys.map(key => {
          // Convert camelCase to snake_case for column names
          return key.replace(/([A-Z])/g, "_$1").toLowerCase();
        });
        ...
        await targetPool.query(`
          INSERT INTO bids (${formattedKeys})
          VALUES (${placeholders})
        `, values);
      ...
      try {
        ...
        for (const row of sourceData) {
          ...
          if (existingRow) {
            ...
            // Insert new row with column name mapping
            ...
            await targetPool.query(`
              INSERT INTO ${tableName} (${formattedKeys})
              VALUES (${placeholders})
            `, values);
